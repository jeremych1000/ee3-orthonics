...
$result2 = db_select("SELECT * FROM `images` WHERE `flight_id`=".$f_id.";");
...
//
	...
	//make result array then compute distance of two image records
	$max_confidence = 50;
	$distance_array = create_distance_array($result2);
	$testt = group_results($distance_array);
	echo '<pre>';
	var_dump($result2);
	...
	$pdf = new PDF();
	$pdf->SetTitle($f_airline.' '.$f_flight_no.' '.$f_date);
	...
	$pdf->Ln(7);
	$pdf->Image($g_zoomout,10,190,-180,-180,'PNG');
	$pdf->Image($g_zoomin,100,190,-180,-180,'PNG');
